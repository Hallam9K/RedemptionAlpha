...
namespace Redemption.Backgrounds.Skies
{
    public class NebSky : CustomSky
    {
        ...
        {
            Texture2D SkyTex = Request<Texture2D>("Redemption/Backgrounds/Skies/NebSky").Value;
            ...
            if (maxDepth >= 3.40282347E+38f && minDepth < 3.40282347E+38f)
            {
                if (!Main.dayTime)
                {
                    if (Main.screenWidth > SkyTex.Width || Main.screenHeight > SkyTex.Height)
                    {
                        ...
                        spriteBatch.Draw(SkyTex, rect, null, Color.White * .9f, 0f, Vector2.Zero, SpriteEffects.None, 0f);
                        if (MoonbeamIntensity > 0f)
                        {
                            float flicker = 1;
                            if (skyColor == Color.Cyan)
                                flicker = Main.rand.NextFloat(.9f, 1.1f);
                            spriteBatch.Draw(SkyTex3, SkyPos, new Rectangle(0, 0, Main.screenWidth, SkyTex3.Height), skyColor with { A = 0 } * MoonbeamIntensity * flicker, 0f, new Vector2(SkyTex.Width >> 1, SkyTex.Height >> 1), 1f, SpriteEffects.None, 1f);
                            if (skyColor == Color.Cyan)
                                spriteBatch.Draw(SkyTex3, SkyPos, new Rectangle(0, 0, Main.screenWidth, SkyTex3.Height), Color.White with { A = 0 } * MoonbeamIntensity * flicker, 0f, new Vector2(SkyTex.Width >> 1, SkyTex.Height >> 1), 1f, SpriteEffects.None, 1f);
                        }
                    }
                    else
                    {
                        Vector2 SkyPos = new(Main.screenWidth / 2, Main.screenHeight / 2);
                        spriteBatch.Draw(SkyTex, SkyPos, null, Color.White * .9f, 0f, new Vector2(SkyTex.Width >> 1, SkyTex.Height >> 1), 1f, SpriteEffects.None, 1f);
                        if (MoonbeamIntensity > 0f)
                        {
                            float flicker = 1;
                            if (skyColor == Color.Cyan)
