...
namespace Redemption.Globals
{
    public static class ElementID
    {
        ...
        public static bool[] ItemShadow = ItemID.Sets.Factory.CreateBoolSet(ItemID.LightsBane, ItemID.PurpleClubberfish, ItemID.NightsEdge, ItemID.TrueNightsEdge, ItemID.DeathSickle, ItemID.NightmarePickaxe, ItemID.WarAxeoftheNight, ItemID.TheBreaker, ItemID.OnyxBlaster);
    ...
    public class ElementalNPC : GlobalNPC
    {
        ...
        public override void ModifyHitByProjectile(Terraria.NPC npc, Projectile projectile, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
        {
            if (!RedeConfigClient.Instance.ElementDisable && !ItemLists.NoElement.Contains(projectile.type))
            {
                #region Elemental Attributes
                float multiplier = 1;
                ElementalEffects(npc, projectile, ref multiplier, ref knockback);
                for (int j = 0; j < npc.GetGlobalNPC<ElementalNPC>().elementDmg.Length; j++)
                {
        ...
        public void ElementalEffects(Terraria.NPC npc, Projectile proj, ref float multiplier, ref float knockback)
        {
            ...
            if (proj.HasElement(ElementID.Wind) && (npc.noGravity || !npc.collideY))
                knockback = (int)((knockback * 1.25f) + 2);
        }
    }
}