...
{
    public class BLTPet : ModProjectile
    {
        ...
        public override bool PreAI()
        {
            ...
            player.dino = false;
            if (Main.player[Projectile.owner].ZoneCorrupt || Main.player[Projectile.owner].ZoneCrimson)
                frameWidth = 2;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            if (frameY is 8)
            {
                Texture2D flowerTex = ModContent.Request<Texture2D>(Texture + "_Flower").Value;
                int height2 = flowerTex.Height / 4;
                int y2 = height2 * frameYFlower;
                Rectangle rect2 = new(0, y2, flowerTex.Width, height2);

                Main.EntitySpriteDraw(flowerTex, Projectile.Center - new Vector2(0, 12) - Main.screenPosition, new Rectangle?(rect2), Projectile.GetAlpha(lightColor), Projectile.rotation, drawOrigin, Projectile.scale, effects, 0);
            }
            return false;
        }
        ...
        {
            if (!player.dead && player.HasBuff(ModContent.BuffType<BLTPetBuff>()))
