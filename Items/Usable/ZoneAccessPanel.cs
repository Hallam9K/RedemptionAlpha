...
namespace Redemption.Items.Usable
{
    ...
    public class ZoneAccessPanel2 : ZoneAccessPanel1
    {
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Lab Access Panel - Beta");
            Tooltip.SetDefault("Opens up the beta sector of the lab");
        }
        ...
        public override bool? UseItem(Player player)
        {
            ...
            {
                Main.NewText("Beta laser security already deactivated", Color.Cyan);
                return true;
            }
            LabArea.labAccess[1] = true;
            string status = "Beta sector laser security has been deactivated";
            if (Main.netMode == NetmodeID.Server)
                ChatHelper.BroadcastChatMessage(NetworkText.FromLiteral(status), Color.Cyan);
            else if (Main.netMode == NetmodeID.SinglePlayer)
                Main.NewText(Language.GetTextValue(status), Color.Cyan);
    ...
    public class ZoneAccessPanel7 : ZoneAccessPanel1
    {
        public override void SetStaticDefaults()
        {
        ...
        public override bool? UseItem(Player player)
        {
            ...
            Texture2D tex = ModContent.Request<Texture2D>("Redemption/WorldGeneration/ALabAccess7", AssetRequestMode.ImmediateLoad).Value;
            Point origin = RedeGen.LabVector.ToPoint();
            Main.QueueMainThreadAction(() =>
            {
                TexGen gen = BaseWorldGenTex.GetTexGenerator(tex, colorToTile);
