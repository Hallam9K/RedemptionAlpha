...
namespace Redemption.Items.Weapons.HM.Melee
{
    public class CrystalGlaive : ModItem
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            {
                if (!Main.dedServ)
                    SoundEngine.PlaySound(new("Redemption/Sounds/Custom/Swoosh1") { Volume = .4f, Pitch = .1f }, player.position);
            ...
            else
            {
                if (Cooldown > 0)
                {
                    switch (Level)
                    {
                        case 0:
                            if (!Main.dedServ)
                                SoundEngine.PlaySound(new("Redemption/Sounds/Custom/Swing1") { Volume = .4f, Pitch = .1f }, player.position);
