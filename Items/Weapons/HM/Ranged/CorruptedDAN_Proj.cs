    ...
    public class CorruptedDAN_Proj : TrueMeleeProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                ...
                {
                    if (Projectile.UseAmmo(AmmoID.Rocket, ref rocket, ref shootSpeed, ref weaponDamage, ref weaponKnockback))
                    {
                        offset = 30;
                        rotOffset = -0.5f;
                        ...
                        for (int i = 0; i < Main.rand.Next(7, 9); i++)
                            Projectile.NewProjectile(Projectile.GetSource_FromAI(), gunPos, RedeHelper.PolarVector(shootSpeed + Main.rand.Next(-4, 5), (Main.MouseWorld - gunPos).ToRotation() + Main.rand.NextFloat(-0.2f, 0.2f)), ModContent.ProjectileType<DAN_Rocket>(), Projectile.damage, Projectile.knockBack, player.whoAmI);
                ...
                if (Projectile.localAI[0] >= 80)
                {
                    ...
                    if (Projectile.localAI[1] == 0)
                    {
                        ...
                        {
                            if (Projectile.UseAmmo(AmmoID.Rocket, ref rocket, ref shootSpeed, ref weaponDamage, ref weaponKnockback, !Main.rand.NextBool(10)))
                            {
                                offset = 12;
                                ...
                                Projectile.NewProjectile(Projectile.GetSource_FromAI(), gunPos, RedeHelper.PolarVector(shootSpeed, Projectile.velocity.ToRotation()), ModContent.ProjectileType<DAN_Rocket>(), (int)(Projectile.damage * 0.75f), Projectile.knockBack, player.whoAmI, 1);
                                for (int i = 0; i < 5; i++)
                                {
