...
namespace Redemption.Items.Weapons.PostML.Melee
{
    public class PZGauntlet_Proj : TrueMeleeProjectile
    {
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                switch (Projectile.ai[0])
                {
                    case 0:
                        if (Timer++ == 0)
                        {
                            player.velocity = RedeHelper.PolarVector(17, (Main.MouseWorld - player.Center).ToRotation());
                        ...
                        if (Timer < 16)
                        {
                            ...
                            Dust dust = Main.dust[Dust.NewDust(player.position, player.width, player.height, DustID.GreenTorch)];
                            dust.position = position;
                            dust.velocity = (player.velocity.RotatedBy(1.57) * 0.33f) + (player.velocity / 4f);
                            dust.position += player.velocity.RotatedBy(1.57);
                            dust.fadeIn = 0.5f;
                            dust.noGravity = true;
                            dust = Main.dust[Dust.NewDust(player.position, player.width, player.height, DustID.GreenTorch)];
                            dust.position = position;
                            dust.velocity = (player.velocity.RotatedBy(-1.57) * 0.33f) + (player.velocity / 4f);
                            dust.position += player.velocity.RotatedBy(-1.57);
                            dust.fadeIn = 0.5f;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Vector2 v = RedeHelper.PolarVector(2, (Projectile.Center - player.Center).ToRotation() + player.fullRotation);
