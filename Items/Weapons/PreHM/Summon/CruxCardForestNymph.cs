...
{
    public class CruxCardForestNymph : ModItem
    {
        public override void SetStaticDefaults()
        {
            // DisplayName.SetDefault("Crux Card: Forest Nymph");
            /* Tooltip.SetDefault("Summons the spirit of a Forest Nymph\n" +
                "Right-click to tug the spirit back to your position, consuming 1 [i:" + ModContent.ItemType<LostSoul>() + "]\n" +
                "Consumes 12 [i:" + ModContent.ItemType<LostSoul>() + "] on use\n" +
        ...
        public override bool CanUseItem(Player player)
        {
            ...
            if (player.altFunctionUse == 2)
            {
                ...
                {
                    NPC npc = Main.npc[n];
                    if (!npc.active || npc.type != ModContent.NPCType<ForestNymph_SS>())
                        continue;
            ...
            if (soul >= 0 && player.inventory[soul].stack >= 12)
            {
                player.inventory[soul].stack -= 12;
                if (player.inventory[soul].stack <= 0)
                    player.inventory[soul] = new Item();
            }
            else
        ...
        public override bool? UseItem(Player player)
        {
            if (player.whoAmI == Main.myPlayer)
            {
                ...
                if (player.altFunctionUse == 2)
                {
                    ...
                    {
                        NPC npc = Main.npc[n];
                        if (!npc.active || npc.type != ModContent.NPCType<ForestNymph_SS>() || npc.ai[3] != player.whoAmI)
                            continue;
                ...
                else
                {
                    ...
                    int type = ModContent.NPCType<ForestNymph_SS>();
        ...
        public override void ModifyTooltips(List<TooltipLine> tooltips)
        {
            ...
            {
                tooltips.Insert(tooltipLocation, new TooltipLine(Mod, "MaxLife", "500 base health"));
                tooltips.Insert(tooltipLocation + 2, new TooltipLine(Mod, "Defense", "5 defense"));
            }
        }
