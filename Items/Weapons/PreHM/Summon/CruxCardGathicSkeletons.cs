...
{
    public class CruxCardGathicSkeletons : ModItem
    {
        ...
        public override bool CanUseItem(Player player)
        {
            ...
            if (player.altFunctionUse == 2)
            {
                ...
                {
                    NPC npc = Main.npc[n];
                    if (!npc.active || (npc.type != ModContent.NPCType<SkeletonWanderer_SS>() && npc.type != ModContent.NPCType<SkeletonDuelist_SS>()))
                        continue;
            ...
            if (soul >= 0 && player.inventory[soul].stack >= 10)
            {
                player.inventory[soul].stack -= 10;
                if (player.inventory[soul].stack <= 0)
                    player.inventory[soul] = new Item();
            }
            else
        ...
        public override bool? UseItem(Player player)
        {
            if (player.whoAmI == Main.myPlayer)
            {
                ...
                if (player.altFunctionUse == 2)
                {
                    ...
                    {
                        NPC npc = Main.npc[n];
                        if (!npc.active || (npc.type != ModContent.NPCType<SkeletonWanderer_SS>() && npc.type != ModContent.NPCType<SkeletonDuelist_SS>()) || npc.ai[3] != player.whoAmI)
                            continue;
                ...
                else
                {
                    ...
                    int type = ModContent.NPCType<SkeletonWanderer_SS>();
                    int type2 = ModContent.NPCType<SkeletonDuelist_SS>();
        ...
        public override void ModifyTooltips(List<TooltipLine> tooltips)
        {
            ...
            {
                tooltips.Insert(tooltipLocation, new TooltipLine(Mod, "MaxLife", "116/124 base health"));
                tooltips.Insert(tooltipLocation + 2, new TooltipLine(Mod, "Defense", "8 defense"));
            }
        }
