...
namespace Redemption.NPCs.Bosses.ADD
{
    public class AkkaHealthBar : ModBossBar
    {
        ...
        public override bool? ModifyInfo(ref BigProgressBarInfo info, ref float life, ref float lifeMax, ref float shield, ref float shieldMax)
        {
            ...
            if (!npc.active || npc.type != NPCType<Akka>())
                return false;
    ...
    public class UkkoHealthBar : ModBossBar
    {
        ...
        {
            NPC npc = Main.npc[info.npcIndexToAimAt];
            if (!npc.active || npc.type != NPCType<Ukko>())
                return false;
        ...
        public override void PostDraw(SpriteBatch spriteBatch, NPC npc, BossBarDrawParams drawParams)
        {
            ...
            if (BigProgressBarSystem.ShowText)
            {
                ...
                if (drawParams.Shield <= 0)
                    return;
                DynamicSpriteFont value = FontAssets.ItemStack.Value;
                Vector2 vector = rectangle.Center.ToVector2() + new Vector2(0, 35);
                vector.Y += 1f;
                string text = "/";
