        ...
        public override void AI()
        {
            ...
            switch (AIState)
            {
                ...
                case ActionState.Intro:
                    ...
                    {
                        if (!Main.dedServ)
                            SoundEngine.PlaySound(SoundLoader.GetLegacySoundSlot(Mod, "Sounds/Custom/ShootChange"), NPC.position);
                        aniType = 2;
                        NPC.netUpdate = true;
                    }
                        ...
                        if (NPC.soundDelay == 0)
                        {
                            if (!Main.dedServ)
                                SoundEngine.PlaySound(SoundLoader.GetLegacySoundSlot(Mod, "Sounds/Custom/Quake")
                                    .WithVolume(MathHelper.Clamp(AITimer2, 0.1f, 2f)), NPC.position);
