...
namespace Redemption.NPCs.Bosses.KSIII
{
    public class KS3_FlashGrenade : ModProjectile
    {
        ...
        public override void Kill(int timeLeft)
        {
            if (!Main.dedServ)
            {
                //SoundEngine.PlaySound(mod.GetLegacySoundSlot(SoundType.Custom, "Sounds/Custom/Zap2").WithPitchVariance(0.1f), (int)projectile.position.X, (int)projectile.position.Y);
            }
            for (int i = 0; i < 15; i++)
            {
