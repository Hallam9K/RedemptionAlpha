        ...
        public override void AI()
        {
            ...
            switch (NPC.ai[0])
            {
                ...
                case 1:
                    ...
                    {
                        RedeHelper.SpawnNPC((int)player.Center.X + Main.rand.Next(70, 180), (int)player.Center.Y - Main.rand.Next(800, 850), ModContent.NPCType<KS3_ScannerDrone>(), ai3: NPC.whoAmI);
                    }
                    if (NPC.ai[2] > 30 && !NPC.AnyNPCs(ModContent.NPCType<KS3_ScannerDrone>()))
                    {
                        if (NPC.ai[1] >= 2)
                        {
                            NPC.Shoot(NPC.Center, ModContent.ProjectileType<KS3_DroneKillCheck>(), 0, Vector2.Zero, false, SoundID.Item1.WithVolume(0f));
                            NPC.ai[0] = 4;
                            NPC.ai[1] = 0;
                            NPC.ai[2] = 0;
                            NPC.netUpdate = true;
