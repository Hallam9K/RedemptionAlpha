...
namespace Redemption.NPCs.Bosses.Keeper
{
    ...
    public class ShadowBolt_Soulless : ModProjectile, ITrailProjectile
    {
        public override string Texture => Redemption.EMPTY_TEXTURE;
        public override void SetStaticDefaults()
        {
        ...
        public void DoTrailCreation(TrailManager tManager)
        {
            tManager.CreateTrail(Projectile, new GradientTrail(new Color(24, 28, 38), Color.Black), new RoundCap(), new DefaultTrailPosition(), 22f, 250f);
            tManager.CreateTrail(Projectile, new GradientTrail(new Color(111, 115, 124), Color.Black), new RoundCap(), new DefaultTrailPosition(), 22f, 200f);
        }
        public override void AI()
        {
            if (Projectile.velocity.Length() < 6)
            ...
            if (Projectile.localAI[0] == 0f)
            {
                for (int m = 0; m < 8; m++)
                {
                    int dustID = Dust.NewDust(Projectile.Center - new Vector2(-1, -1), 2, 2, DustID.AncientLight, 0f, 0f, 100, Color.White, 2f);
                    Main.dust[dustID].velocity = BaseUtility.RotateVector(default, new Vector2(4f, 0f), m / (float)8 * 6.28f);
                    Main.dust[dustID].noLight = false;
                    Main.dust[dustID].noGravity = true;
                }
            ...
            if (Main.rand.NextBool(2))
                ParticleManager.NewParticle(Projectile.Center, Vector2.Zero, new GlowParticle2(), new Color(24, 28, 38), 1f, .45f, Main.rand.Next(10, 20));
            if (Main.rand.NextBool(2))
                ParticleManager.NewParticle(Projectile.Center, Vector2.Zero, new GlowParticle2(), new Color(111, 115, 124), 1f, .45f, Main.rand.Next(10, 20));
