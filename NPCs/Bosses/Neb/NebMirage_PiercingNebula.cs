...
{
    public class NebMirage_PiercingNebula : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (!npc.active || (npc.type != NPCType<Nebuleus>() && npc.type != NPCType<Nebuleus_Clone>()))
            {
                Projectile.alpha += 10;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            int height2 = Nebuleus.armsPiercingAni.Value.Height / 9;
            ...
            for (int k = Projectile.oldPos.Length - 1; k >= 0; k -= 1)
            {
                ...
                Main.EntitySpriteDraw(Nebuleus.armsPiercingAni.Value, Projectile.oldPos[k] + new Vector2(Projectile.width / 2, Projectile.height / 2) + new Vector2(0, 0) - Main.screenPosition, new Rectangle?(new Rectangle(0, y2, Nebuleus.armsPiercingAni.Value.Width, height2)), Main.DiscoColor * (0.5f * alpha) * Projectile.Opacity, oldrot[k], drawOrigin, Projectile.scale, effects, 0);
            }
            ...
            Main.EntitySpriteDraw(Nebuleus.wings.Value, Projectile.Center - Main.screenPosition, new Rectangle?(rect), Projectile.GetAlpha(Color.White), Projectile.rotation, drawOrigin, Projectile.scale, effects, 0);

            Main.spriteBatch.End();
            Main.spriteBatch.BeginAdditive();

            Main.EntitySpriteDraw(Nebuleus.armsPiercingAni.Value, drawCenter + new Vector2(0, 0) - Main.screenPosition, new Rectangle?(new Rectangle(0, y2, Nebuleus.armsPiercingAni.Value.Width, height2)), Projectile.GetAlpha(lightColor), Projectile.rotation, drawOrigin, Projectile.scale, effects, 0);
            Main.EntitySpriteDraw(Nebuleus.armsPiercingGlow.Value, drawCenter + new Vector2(0, 0) - Main.screenPosition, new Rectangle?(new Rectangle(0, y2, Nebuleus.armsPiercingAni.Value.Width, height2)), Projectile.GetAlpha(Color.White), Projectile.rotation, drawOrigin, Projectile.scale, effects, 0);
