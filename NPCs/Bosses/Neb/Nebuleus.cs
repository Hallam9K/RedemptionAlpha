            ...
            switch ((int)NPC.ai[0])
            {
                ...
                case 8:
                    ...
                    else
                    {
                        ...
                        if (NPC.ai[2] >= 2660)
                        {
                            NPC.Shoot(NPC.Center, ModContent.ProjectileType<ProjDeath>(), 0, Vector2.Zero, false, SoundID.Item1 with { Volume = 0 });
                            NPC.life = 1;
                            NPC.ai[2] = 0;
                            NPC.ai[0] = 9;
                            NPC.netUpdate = true;
                ...
                case 10: // Phase 2
                    ...
                    if (!Main.dedServ)
                    {
                        ...
                        if (NPC.ai[2] == 480)
                            SoundEngine.PlaySound(new("Redemption/Sounds/Custom/Transformation"), NPC.position);
                    }
                    if (NPC.ai[2] == 573)
                        NPC.Shoot(NPC.Center, ModContent.ProjectileType<Transition>(), 0, Vector2.Zero, false, SoundID.Item1 with { Volume = 0 });
                    if (NPC.ai[2] == 670)
                        NPC.Shoot(NPC.Center, ModContent.ProjectileType<ShockwaveBoom2>(), 0, Vector2.Zero, false, SoundID.Item1 with { Volume = 0 });
                    if (NPC.ai[2] > 700)
                    {
                        if (RedeBossDowned.nebDeath < 5)
                            RedeBossDowned.nebDeath = 5;
                ...
                case 11: // Spared
                    ...
                    if (RedeWorld.alignment >= 0)
                    {
                        NPC.dontTakeDamage = false;
                        if (!Main.dedServ)
                            SoundEngine.PlaySound(new("Redemption/Sounds/Custom/Teleport1"), NPC.position);
                        if (RedeBossDowned.nebDeath < 5)
                            RedeBossDowned.nebDeath = 5;
