...
namespace Redemption.NPCs.Bosses.Obliterator
{
	...
	public class OO : ModNPC
	{
        ...
        public List<int> AttackList = new() { 0, 1, 2, 3, 4, 5 };
			...
			switch (AIState)
			{
					...
					switch (TimerRand)
					{
						...
						case 1:
							...
							if (AITimer == 120)
							{
								...
								Dialogue d1 = new(NPC, "Yo.", Colors.RarityRed, Color.DarkRed, CustomSounds.Voice1 with { Volume = 0 }, 3, 60, 0, false); // 69
							...
							if (AITimer == 400)
							{
								Dialogue d1 = new(NPC, "I guess I can't fool you twice,[10] huh.", Colors.RarityRed, Color.DarkRed, voice, 2, 100, 0, false); // 182
								Dialogue d2 = new(NPC, "So much for a surprise attack...", Colors.RarityRed, Color.DarkRed, voice, 2, 118, 0, false); // 182
							...
							if (AITimer == 582)
							{
								...
								chain.Add(new(NPC, "Hang on,[10] I got a call from Girus.", Colors.RarityRed, Color.DarkRed, voice, 2, 100, 0, false)) // 166
									 .Add(new(NPC, "'I wasted too much energy too quickly?'", Colors.RarityRed, Color.DarkRed, voice, 2, 100, 0, false)) // 178
									 .Add(new(NPC, "'I'm an idiot?'", Colors.RarityRed, Color.DarkRed, voice, 2, 100, 0, false)) // 130
									 .Add(new(NPC, "You're scrapping my personality drive after this fight?", Colors.RarityRed, Color.DarkRed, voice, 2, 100, 0, false)) // 210
									 .Add(new(NPC, "Ah well,[10] request accepted...", Colors.RarityRed, Color.DarkRed, voice, 2, 100, 0, false)) // 156
									 .Add(new(NPC, "Anyway...", Colors.RarityRed, Color.DarkRed, voice, 2, 100, 30, true)); // 148
								HeadFrameY = 2;
								TextBubbleUI.Visible = true;
				...
				case ActionState.Begin:
					...
					if (AITimer == 60)
					{
						...
						Dialogue d1 = new(NPC, "Ready for obliteration?", Colors.RarityRed, Color.DarkRed, voice, 2, 100, 30, true); // 176
						TextBubbleUI.Visible = true;
						TextBubbleUI.Add(d1);
					}
					if (AITimer >= 236)
                        ...
                        case 5:
                            ...
                            if (NPC.life < (int)(NPC.lifeMax * 0.4f))
                            {
                                if ((NPC.life < (int)(NPC.lifeMax * 0.2f) ? AITimer % 8 == 0 : AITimer % 10 == 0) && AITimer < 300)
                                    NPC.Shoot(HandPos, ModContent.ProjectileType<OO_Fingerflash>(), 90, Vector2.Zero, false, CustomSounds.Laser1, NPC.whoAmI);
                            }
                            else
                            {
                            ...
                            else
                            {
                                ...
                                if (AITimer == 305)
                                {
                                    Dialogue d1 = new(NPC, null, null, null, Colors.RarityRed, Color.DarkRed, voice, null, "Eye beam!", 2, 70, 30, true);
                                    TextBubbleUI.Visible = true;
		...
		private void DespawnHandler()
		{
			...
			if (!player.active || player.dead)
			{
				...
				else if (NPC.ai[0] == -1)
				{
					...
					{
						DialogueChain chain = new();
						chain.Add(new(NPC, "Alright,[10] target eliminated.", Colors.RarityRed, Color.DarkRed, voice, 2, 100, 0, false)) // 154
							 .Add(new(NPC, "Returning to base...", Colors.RarityRed, Color.DarkRed, voice, 2, 100, 30, true)); // 170
				...
				else
				{
					if (NPC.ai[0] != -2)
					{
						Dialogue d1 = new(NPC, "Target eliminated...", Colors.RarityRed, Color.DarkRed, voice, 2, 100, 30, true); // 150
