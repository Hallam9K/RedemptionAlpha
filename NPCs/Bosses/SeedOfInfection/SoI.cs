...
namespace Redemption.NPCs.Bosses.SeedOfInfection
{
    ...
    public class SoI : ModNPC
    {
                ...
                case ActionState.Attacks:
                    switch (ID)
                    {
                            ...
                            if (++AITimer == 80)
                            {
                                for (int i = 0; i < 3; i++)
                                {
                                    ...
                                    NPC.Shoot(NPC.Center, ModContent.ProjectileType<SoI_ShardShot>(), NPC.damage, RedeHelper.PolarVector(8,
                                        (player.Center - NPC.Center).ToRotation() + MathHelper.ToRadians(rot - 20)), false, SoundID.Item42);
                                }
                                if (NPC.life < NPC.lifeMax / 2)
                                {
                                    for (int i = 0; i < 2; i++)
                                    {
                                        int rot = 80 * i;
                                        NPC.Shoot(NPC.Center, ModContent.ProjectileType<SoI_ShardShot>(), NPC.damage, RedeHelper.PolarVector(8,
                                            (player.Center - NPC.Center).ToRotation() + MathHelper.ToRadians(rot - 40)), false, SoundID.Item42);
                                    }
                                }
                            }
                            if (AITimer >= 120)
                        ...
                        case 2:
                            ...
                            if (AITimer == 90)
                            {
                                for (int i = 0; i < (NPC.life < NPC.lifeMax / 2 ? 12 : 8); i++)
                                {
                                    NPC.Shoot(NPC.Center, ModContent.ProjectileType<SoI_ShardShot>(), NPC.damage,
                                        new Vector2(Main.rand.Next(-8, 9), Main.rand.Next(-14, -4)), false, SoundID.Item42, "", 1);
                                }
                            }
                            if (AITimer >= 150)
                            {
                        ...
                        case 3:
                            if (NPC.life < NPC.lifeMax / 2)
                            {
                                ...
                                if (++AITimer >= 100 && AITimer % 50 == 0 && AITimer <= (Main.expertMode ? 250 : 200))
                                {
                                    ...
                                    {
                                        NPC.Shoot(NPC.Center, ModContent.ProjectileType<SoI_ToxicSludge>(), NPC.damage,
                                            RedeHelper.PolarVector(12, (player.Center - NPC.Center).ToRotation() + Main.rand.NextFloat(-0.06f, 0.06f)), false, SoundID.Item60);
                                    }
                                }
                                if (AITimer >= 300)
                                {
                        ...
                        case 4:
                            if (NPC.life < NPC.lifeMax / 2)
                            {
                                ...
                                if (AITimer >= 20 && AITimer <= 240)
                                {
                                    if (Main.rand.NextBool(10))
                                        NPC.Shoot(NPC.Center, ModContent.ProjectileType<SoI_XenomiteShot>(), NPC.damage,
                                            new Vector2(Main.rand.Next(-10, 11), Main.rand.Next(-14, -4)), false, SoundID.Item17);
                                }
                        ...
                        case 5:
                            NPC.rotation += 0.03f;
                            NPC.velocity *= 0.9f;
                            if (++AITimer >= 30 && AITimer % 30 == 0 && AITimer <= 90)
                            {
                                SoundEngine.PlaySound(SoundID.NPCDeath13, NPC.position);
                        ...
                        case 6:
                            if (NPC.life < NPC.lifeMax / 4)
                            {
                                ...
                                if (++AITimer >= 30 && AITimer <= 200)
                                {
                                    ...
                                    {
                                        NPC.Shoot(NPC.Center, ModContent.ProjectileType<SoI_ToxicSludge>(), NPC.damage,
                                            new Vector2(Main.rand.Next(-8, 9), Main.rand.Next(-14, 15)), false, SoundID.Item17);
                                    }
                                }
                                if (AITimer < 160)
                                    TimerRand += 0.08f;
                        ...
                        case 7:
                            ...
                            {
                                if (AITimer < 70)
                                    NPC.MoveToVector2(TopPos, 9);
                                else
                                    NPC.velocity *= 0.7f;

                                if (++AITimer == 70)
                                    NPC.Shoot(NPC.Center, ModContent.ProjectileType<SeedLaser>(), (int)(NPC.damage * 1.4f), new Vector2(0, 10), false, SoundID.Item103, ai0: NPC.whoAmI);
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            if (!FreakOut)
                spriteBatch.Draw(EyeTex, NPC.Center - screenPos, NPC.frame, NPC.GetAlpha(drawColor), 0, NPC.frame.Size() / 2, NPC.scale, effects, 0);
