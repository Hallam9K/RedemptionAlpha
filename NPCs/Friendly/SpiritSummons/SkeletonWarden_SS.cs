...
namespace Redemption.NPCs.Friendly.SpiritSummons
{
    public class SkeletonWarden_SS : SkeletonBase
    {
        ...
        {
            if (NPC.ai[3] != -1)
                typeName = Main.player[(int)NPC.ai[3]].name + "'s Skeleton Warden";
        }
        ...
        public override void AI()
        {
            ...
            if (!NPC.RedemptionGuard().GuardBroken)
            {
                for (int i = 0; i < Main.maxProjectiles; i++)
                {
                    ...
                    if (!projectile.active || projectile.damage <= 0 || projectile.friendly || projectile.ProjBlockBlacklist())
                        continue;
        ...
        public int GetNearestNPC(bool nearestUndead = false)
        {
            ...
            for (int i = 0; i < Main.maxNPCs; i++)
            {
                ...
                if (!target.active || target.whoAmI == NPC.whoAmI || target.type == ModContent.NPCType<SkeletonWarden_SS>() || target.dontTakeDamage || target.type == NPCID.OldMan)
                    continue;
