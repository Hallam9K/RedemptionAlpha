...
{
    public class ShadestoneLiftBig : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            Rectangle buttonRect = new((int)NPC.position.X + 64, (int)NPC.position.Y - 14, 32, 18);
            for (int i = 0; i < Main.maxPlayers; i++)
            {
                ...
                if (!Main.dedServ)
                    SoundEngine.PlaySound(CustomSounds.Switch1, NPC.position);
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
        {
            ...
            Texture2D button = ModContent.Request<Texture2D>("Redemption/NPCs/Soulless/ShadestoneLift_Button").Value;
            Vector2 drawOrigin = new(texture.Width / 2, texture.Height / 2);
            Main.EntitySpriteDraw(button, NPC.Center - new Vector2(-60, buttonY - 16) - new Vector2(0, NPC.velocity.Y * 2) - screenPos, null, NPC.GetAlpha(drawColor), NPC.rotation, drawOrigin, NPC.scale, SpriteEffects.None, 0);
            Main.EntitySpriteDraw(texture, NPC.Center - new Vector2(0, 10 + NPC.velocity.Y * 2) - screenPos, null, NPC.GetAlpha(drawColor), NPC.rotation, drawOrigin, NPC.scale, SpriteEffects.None, 0);
            return false;
        }
    }
}