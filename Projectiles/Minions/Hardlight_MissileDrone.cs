        ...
        public override void AI()
        {
            ...
            if (Projectile.localAI[0] > 120 && Projectile.localAI[0] < 500 && shotCount < 4)
            {
                ...
                if (getNPC != -1 && Projectile.localAI[0] % 30 == 0 && Projectile.owner == Main.myPlayer)
                {
                    ...
                    Projectile.NewProjectile(Projectile.InheritSource(Projectile), Projectile.Center, RedeHelper.PolarVector(10, (Main.npc[getNPC].Center - Projectile.Center).ToRotation() + Main.rand.NextFloat(0.2f, 0.2f)), ModContent.ProjectileType<Hardlight_SlayerMissile>(), 96, 4, player.whoAmI, Projectile.whoAmI);
                    shotCount++;
                }
            }
