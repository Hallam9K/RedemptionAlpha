...
{
    public class FlakGrenade_Bee : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            {
                for (int i = 0; i < 4; i++)
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, RedeHelper.SpreadUp(7), ProjectileType<FlakGrenade_Frag>(), Projectile.damage / 4, 1, Main.myPlayer, 3);

                for (int i = 0; i < Main.rand.Next(2, 4); i++)
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, RedeHelper.Spread(7), ProjectileID.Bee, Projectile.damage / 4, 1, Main.myPlayer, 3);
            }
            for (int i = 0; i < 15; i++)
            {
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                Main.dust[dust].velocity *= Main.rand.NextFloat(1, 3);
                Main.dust[dust].noGravity = true;
            }
            ...
            int radius = 80;
            RedeHelper.NPCRadiusDamage(radius, Projectile, Projectile.damage, Projectile.knockBack, Projectile.CritChance);
        }
