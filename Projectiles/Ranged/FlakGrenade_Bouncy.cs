...
namespace Redemption.Projectiles.Ranged
{
    public class FlakGrenade_Bouncy : ModProjectile
    {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            {
                for (int i = 0; i < 4; i++)
                    Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, RedeHelper.SpreadUp(7), ProjectileType<FlakGrenade_Frag>(), Projectile.damage / 4, 1, Main.myPlayer, 1);
            }
            for (int i = 0; i < 15; i++)
            {
