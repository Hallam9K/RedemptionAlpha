...
{
	public class CorruptWastelandVine : ModTile
	{
		...
		public override bool TileFrame(int i, int j, ref bool resetFrame, ref bool noBreak)
		{
			...
			if (type == ModContent.TileType<IrradiatedCorruptGrassTile>() || type == Type) {
				return true;
			}
		...
		public override void RandomUpdate(int i, int j)
		{
			...
			if (WorldGen.genRand.NextBool(15) && !tileBelow.HasTile && tileBelow.LiquidType != LiquidID.Lava) {
				...
				while (yTest > j - 10) {
					...
					else if (!testTile.HasTile || testTile.TileType != ModContent.TileType<IrradiatedCorruptGrassTile>())
                    {
						yTest--;
						continue;
					}
        ...
        public override bool PreDraw(int i, int j, SpriteBatch spriteBatch) 
        {
			...
			spriteBatch.Draw(ModContent.Request<Texture2D>("Redemption/Tiles/Plants/CorruptWastelandVine").Value, drawPos + zero - new Vector2(xOff, 0), realSource, new Color(col.R, col.G, col.B, 255), 0f, new Vector2(0, 0), 1f, SpriteEffects.None, 0f);
            return false;
        }
