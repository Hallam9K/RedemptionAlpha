...
namespace Redemption.Tiles.Plants
{
	public class PurityWastelandVine : ModTile
	{
		...
		public override void RandomUpdate(int i, int j)
		{
			...
			if (WorldGen.genRand.NextBool(15) && !tileBelow.IsActive && tileBelow.LiquidType != LiquidID.Lava) {
				...
				while (yTest > j - 10) {
					...
					else if (!testTile.IsActive || testTile.type != ModContent.TileType<IrradiatedGrassTile>())
                    {
						yTest--;
						continue;
					}
        ...
        public override bool PreDraw(int i, int j, SpriteBatch spriteBatch) 
        {
			...
			spriteBatch.Draw(ModContent.Request<Texture2D>("Redemption/Tiles/Plants/PurityWastelandVine").Value, drawPos + zero - new Vector2(xOff, 0), realSource, new Color(col.R, col.G, col.B, 255), 0f, new Vector2(0, 0), 1f, SpriteEffects.None, 0f);
            return false;
        }
