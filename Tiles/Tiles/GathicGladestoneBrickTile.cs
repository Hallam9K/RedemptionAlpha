...
namespace Redemption.Tiles.Tiles
{
    public class GathicGladestoneBrickTile : ModTile
    {
        ...
        {
            if (!Framing.GetTileSafely(i, j - 1).IsActive && Main.tile[i, j].IsActive && Main.rand.NextBool(15) && Main.tile[i, j - 1].LiquidAmount == 0)
            {
            ...
            if (!Framing.GetTileSafely(i, j + 1).IsActive && Main.tile[i, j].IsActive && Main.rand.NextBool(25) && Main.tile[i, j + 1].LiquidAmount == 0)
            {
