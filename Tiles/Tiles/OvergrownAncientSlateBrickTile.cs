...
{
    public class OvergrownAncientSlateBrickTile : ModTile
    {
        ...
        public override void RandomUpdate(int i, int j)
        {
            ...
            if (!tileAbove.HasTile && !tileAbove2.HasTile && Main.tile[i, j].HasTile && Main.rand.NextBool(400) && Main.tile[i, j - 1].LiquidAmount == 0)
            {
            ...
            if (Main.rand.NextBool(40))
                WorldGen.SpreadGrass(i + Main.rand.Next(-1, 1), j + Main.rand.Next(-1, 1), ModContent.TileType<AncientSlateBrickTile>(), Type, false);
        }
    }
}