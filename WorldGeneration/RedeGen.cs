            ...
            if (ShiniesIndex2 != -1)
            {
                ...
                tasks.Insert(ShiniesIndex2 + 6, new PassLegacy("Portals", delegate (GenerationProgress progress, GameConfiguration configuration)
                {
                    ...
                    while (!placed)
                    {
                        ...
                        int attempts = 0;
                        while (attempts < 50000 && !WorldGen.structures.CanPlace(new Rectangle((int)origin.X, (int)origin.Y, 60, 82)))
                        {
                ...
                tasks.Insert(ShiniesIndex2 + 11, new PassLegacy("Slayer Ship", delegate (GenerationProgress progress, GameConfiguration configuration)
                {
                    ...
                    bool inSpawn = false;
                    bool failed = false;
                    while ((attempts < 50000 && !WorldGen.structures.CanPlace(new Rectangle((int)origin.X, (int)origin.Y, 133, 58))) || inSpawn)
                    {
                        switch (checkType)
                        {
                            case 0:
